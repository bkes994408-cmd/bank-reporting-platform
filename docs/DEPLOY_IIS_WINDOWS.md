...
2. IIS 前端站提供靜態檔
3. URL Rewrite 將 `/api/*` 轉發到後端站（預設 `http://127.0.0.1:5001`）
4. 前端 `API Base` 設成 `/api`

> `deploy-backend.ps1` 預設會把後端站台綁在 `127.0.0.1:5001`，避免 API 直接對外暴露。
> 若你的架構需要改成內網 IP 或對外開放，請改用 `-BindingIp` 參數（例如 `10.0.0.12` 或 `*`）。
...
```powershell
pwsh .\scripts\iis\deploy-backend.ps1 -SiteName BankReporting-Api -AppPoolName BankReporting-Api-AppPool -PublishRoot D:\Sites\BankReporting -Port 5001 -BindingIp 127.0.0.1
```

### 7.3 安裝前端到 IIS

```powershell
pwsh .\scripts\iis\deploy-frontend.ps1 -SiteName BankReporting-Web -SiteRoot D:\Sites\BankReporting\frontend -BackendOrigin http://127.0.0.1:5001 -ApiBaseUrl /api -UseReverseProxy
```
...
`deploy-frontend.ps1` 只會建立 IIS Site（預設 `-Port 443`），**不會自動綁憑證**。

請在部署後擇一完成：

1. 用 IIS Manager：
   - Sites → `BankReporting-Web` → Bindings...
   - 新增/編輯 `https`，Port `443`，選正確憑證，Host name/SNI 依網域設定

2. 或用 PowerShell（系統管理員）：

```powershell
# 先在 LocalMachine\My 找到憑證 thumbprint
Get-ChildItem Cert:\LocalMachine\My | Select-Object Subject, Thumbprint, NotAfter

# 將 site 的 https binding 綁到指定憑證（示例）
$thumbprint = 'PUT_CERT_THUMBPRINT_HERE'
$siteName = 'BankReporting-Web'
...
Get-Item "cert:\LocalMachine\My\$thumbprint" | New-Item "IIS:\SslBindings\0.0.0.0!443" -Force | Out-Null
```

完成後請確認：
- `https://reporting.example.com/` 憑證鏈正確
- Browser 不出現憑證警告
- `Secure` cookie 可正常送出
